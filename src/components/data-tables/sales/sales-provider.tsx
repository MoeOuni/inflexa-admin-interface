...
interface SalesTableContextProps {
  featureFlags: FeatureFlagValue[];
  setFeatureFlags: React.Dispatch<React.SetStateAction<FeatureFlagValue[]>>;
}

const OrdersTableContext =
  React.createContext<SalesTableContextProps>({
    featureFlags: [],
    setFeatureFlags: () => {},
  });

export const useSalesTable = (): SalesTableContextProps => {
  const context = React.useContext(OrdersTableContext);
  
  if (!context) {
    throw new Error('useSalesTable must be used within a SalesTableProvider');
  }
  return context;
};

export function SalesTableProvider({
  children,
