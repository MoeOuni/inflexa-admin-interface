...
export const OrderFormSchema = z
  .object({
    ...
    shippingAddress: z.object({
      ...
      country: z.string().min(1, {
        message: 'Shipping Country is required and cannot be empty',
      }),
    }),
  ...
  .refine(
    (data) => {
      if (data.payment === 'payment_on_delivery') {
        return (
          data.delivery?.deliveryMan?.name &&
          data.delivery?.deliveryMan?.registrationNumber &&
          data.delivery?.deliveryMan?.phone &&
    ...
    {
      message:
        'Delivery and Delivery Price are required when payment is "Payment on delivery"',
      path: ['delivery', 'deliveryPrice'],
    },
  );
